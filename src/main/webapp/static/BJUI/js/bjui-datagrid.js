...
+function ($) {
    ...
    Datagrid.DEFAULTS = {
        ...
        myHeight        : 0,
        afterLoadData   : null      // Function - after loadData method add by zxm on 2017-05-25
    }
        ...
        var tools = {
            ...
            loadData: function(data, refreshFlag) {
                ...
                $.ajax({url:url, data:data || {}, type:options.loadType, cache:options.cache || false, dataType:dataType, success:function(response) {
                        ...
                        // 表格加载完数据后，执行回调方法。add by zxm on 2017-05-25
                        if (options.afterLoadData) {
                        	options.afterLoadData(data, refreshFlag)
                        }
                    }
                })
            },
            ...
            appendColumns: function() {
                ...
                that.editBtnsColumn   = {name:'gridEdit', gridEdit:true, width:50, minWidth:50, label:'操作', align:'center', menu:false, edit:false, hide:false, quicksort:false}
            },
            // create Thead
            ...
            setBoxbH: function(height) {
                ...
                if(options.myHeight==1){
                    that.$boxB.height(d_h)
                }
                else{
...
//                that.$boxB.height(boxH)
            ...
            showSubMenu: function($li, $menu, $submenu) {
                ...
                $li.hover(function() {
                    ...
                    left       = $menu.offset().left - that.$grid.offset().left - 1
                    animate_op = {left:'100%'}
                    var t_h = $('.tabsPageContent').height()-20;
                    var menu_h = parseInt($menu.css('top'))+73;
                    ////显示隐藏列功能 加上滚动条 by 马超 20160802
//                    var su_h = t_h - menu_h;
//                    if(default_height>su_h){
//                        $('.datagrid-showhide-box').width(default_width+18);
//                        $('.datagrid-showhide-box').height(su_h-5);
//                        $('.datagrid-showhide-box').css('overflow-y','auto');
//                    }
//                    else{
//                        $('.datagrid-showhide-box').width(default_width);
//                        $('.datagrid-showhide-box').height(default_height);
//                        $('.datagrid-showhide-box').css('overflow-y','hidden');
//                    }
                    //
                    if ($menu.hasClass('position-right') || (boxWidth - left < width + submenu_width)) {
            ...
            createShowhide: function() {
                ...
                if (!that.$showhide) {
                    ...
                    //显示隐藏列功能 加上滚动条 by 马超 2016/10/24
                    that.$showhide['_width'] = $showhide.width();
                    that.$showhide['_height'] = $showhide.height();
                }
            },
            // column - display/hide
            ...
            jumpPage: function(pageCurrent, pageSize) {
                ...
                if (options.local == 'remote') {
                	/** 添加排序和过滤参数 modified by zxm 2015-11-18 **/
                	var $th,model,$thtmp
                	$.each(that.columnModel, function(i, m) {
                		$thtmp = m.th
                		if($thtmp.find('> div > .datagrid-label > i').length > 0){
                			model = m
                			return
                		}
                	})
                	...
                	var direction, filterDatas = this.getRemoteFilterData(true)
                	if($th){
                		var $isort = $th.find('> div > .datagrid-label > i')
                        if($isort.length > 0) {
                    		if (options.sortAll) {
                    			if ($isort.attr("class") == 'fa fa-long-arrow-down') {
                                    direction = 'desc'
                                } else if ($isort.attr("class") == 'fa fa-long-arrow-up'){
                                    direction = 'asc'
                                }
                        		filterDatas.push({name:BJUI.pageInfo.orderField, value:model.name})
                        		filterDatas.push({name:BJUI.pageInfo.orderDirection, value:direction})
                    		}
                    	}
                	}
            ...
            quickSort: function(model) {
                ...
                if (options.sortAll) {
                    if (options.local == 'remote') {
                    	/** 添加过滤参数 modified by zxm 2015-11-18 **/
                        postData = this.getRemoteFilterData(true)
                        postData.push({name:BJUI.pageInfo.orderField, value:name})
                		postData.push({name:BJUI.pageInfo.orderDirection, value:direction})
            ...
            quickFilter: function(model, filterDatas,initCallBack) {
                if (that.isDom) {
                    if (options.local != 'remote') {
                ...
                if (options.filterAll) {
                    if (options.local == 'remote') {
                    	/** 添加排序参数 modified by zxm 2015-11-18 **/
                    	var $th2,model2,$thtmp
                    	$.each(that.columnModel, function(i, m) {
                    		$thtmp = m.th
                    		if($thtmp.find('> div > .datagrid-label > i').length > 0){
                    			model2 = m
                    	...
                    	var direction, postData = tools.getRemoteFilterData()
                    	if($th2){
                        	var $isort = $th2.find('> div > .datagrid-label > i')
                        	if($isort.length > 0) {
                        		if (options.sortAll) {
    	                    		if ($isort.attr("class") == 'fa fa-long-arrow-down') {
    	                                direction = 'desc'
    	                    		...
    	                    		postData.push({name:BJUI.pageInfo.orderField, value:model2.name})
    	                    		postData.push({name:BJUI.pageInfo.orderDirection, value:direction})
                        		}
                        	}
                    	}
                    	...
                    	/** 如果初始化回调方法存在则执行回调方法 modified by zxm 2017-1-11 **/
                    	if(initCallBack){
                    		initCallBack(postData, true)
                    	} else {
            ...
            getRemoteFilterData: function(isPaging) {
                ...
                if (that.filterDatas && !$.isEmptyObject(that.filterDatas)) {
                    $.each(that.filterDatas, function(name, v) {
                        ...
                        /** 给andor添加前缀，modify by zxm 2015-09-28 **/
                        if (v.datas.andor)
                            filterDatas.push({name:name +'.andor', value:v.datas.andor})
            ...
            contextmenuH: function() {
                ...
                that.$tableH.on('contextmenu', function(e) {
                    ...
                    //显示隐藏列功能 加上滚动条 by 马超 2016/10/24
                    var t_h = $(window).height()-posY;
                    if( that.$showhide['_height']>t_h){
                        that.$showhide.width(that.$showhide['_width']+18);
                        that.$showhide.height($(window).height()-posY-30);
                        that.$showhide.css('overflow-y','auto');
                    }
                    else{
                        that.$showhide.width(that.$showhide['_width']);
                        that.$showhide.height(that.$showhide['_height']+6);
                        that.$showhide.css('overflow-y','hidden');
                    }
                    //
                    that.$showhide
                        .appendTo('body')
    ...
    Datagrid.prototype.init = function() {
        ...
        if (that.isDom) {
            ...
            that.$tbody = null 
            /** 如果列模型里设置了filterDatas，则执行过滤操作 modified by zxm 2017-1-9/1-11 **/
            if (options.data || options.dataUrl) {
            	/** 调用时增加参数 modified by zxm 2017-2-23 **/
            	//tools.createTbody()
            	tools.createTbody(true)
            	var initFilterData;
            	$.each(that.columnModel, 
            		function(i, m) {
	            		if(m.filterDatas){
	            			tools.quickFilter(m, m.filterDatas, function(postData, isPage){
	            				initFilterData = postData;
	            			})
	            			m.th.addClass('filter-active')
	            		}
	            	})
	           /** 由于初始化时数据异步加载，所以延迟200毫秒执行 **/
	           setTimeout(function(){
	        	   if(initFilterData){
	        		   tools.loadData(initFilterData, true)
	        	   }
	           }, 200)
            }
            //if (options.data || options.dataUrl) tools.createTbody()
    ...
    /** 
     * 在不清除过滤条件和排序条件的情况下，刷新数据
     * modify by zxm Date:2016-04-21
     * **/
    Datagrid.prototype.refreshNoClear = function() {
    	var that = this, options = that.options, tools = that.tools, isDom = that.isDom, pageInfo = BJUI.pageInfo, paging = that.paging, postData = {}
    	...
    	var direction, filterDatas = tools.getRemoteFilterData(true)
    	if($th){
    		var $isort = $th.find('> div > .datagrid-label > i')
            if($isort.length > 0) {
        		if (options.sortAll) {
    	...
    	tools.loadData(filterDatas, true)
    }
                                        ...
                                        if (opts.options && opts.options.url) {
                                            if (opts.type == 'dialog') {
                                               ...
                                               /* $.fileDownload(opts.options.url, {
                                            	...
                                            	/** 取得过滤参数 modified by zxm 2016-5-17 **/
                                            	var filterDatas
                                            	var $th,model,$thtmp
                                            	$.each(that.columnModel, function(i, m) {
                                            		$thtmp = m.th
                                            	...
                                            	var direction, filterDatas = tools.getRemoteFilterData(false)
                                            	if($th){
                                            		var $isort = $th.find('> div > .datagrid-label > i')
                                                    if($isort.length > 0) {
                                                		if (options.sortAll) {
    ...
    Datagrid.prototype.colMenu = function() {
        ...
        $ths.each(function() {
            ...
            if (!$btnBox.length) {
            	/** 如果列模型里设置了filterDatas，则过滤按钮图标应该不同 modified by zxm 2017-1-9 **/
            	if(model.filterDatas){
            		/** 如果列模型里设置了filterDatas，如果filterDatas没有值图标不被选中 modified by zc 2017-1-13 **/
            		if(model.filterDatas.valA || model.filterDatas.valB){
            			$btn    = $('<button class="btn btn-default"><i class="fa fa-filter"></button>')
            		}else{
            			$btn    = $('<button class="btn btn-default"><i class="fa fa-bars"></button>')
            			model.th.removeClass("filter-active")
            		}
            	} else {
            		$btn    = $('<button class="btn btn-default"><i class="fa fa-bars"></button>')
            	}
        ...
        if (filterFlag) {
            ...
            if (!$filter || !$filter.length) {
                ...
                var index = $th.data('index'), model = that.columnModel[index], type = model.type || 'string', operator = model.operator || [],
                    ...
                    $valA, $valB
                  //增加datagrid，回车可直接查询功能 by 马超 2016/8/16
                //$input.removeAttr('data-rule').attr('size', 10).addClass('filter-input')
                    $input.removeAttr('data-rule').attr({'size': 10,'onkeydown':'select_a(event,this)'}).addClass('filter-input')
                
                if (type == 'string' || type == 'lookup') {
                	//增加“不包含”筛选 by 马超 2016/8/11
                    if (!operator.length) operator = ['like','not like','=', '!=']
                } else if (type == 'number' || type == 'int' || type == 'spinner') {
                    if (type == 'spinner') $input.removeAttr('data-toggle')
                    if (!operator.length) operator = ['=', '!=', '>', '<', '>=', '<=']
                } else if (type == 'date') {
                	/** 日期类型添加大于小于操作符 modified by zxm 2015-11-23 **/
                    if (!operator.length) operator = ['=', '!=', '>', '<', '>=', '<=']
                ...
                for (var i = 0; i < operator.length; i++) {
                	...
                	if(operator[i]=='='){
                		...
                		show_name = "小于等于";
                	}else if(operator[i]=='like'){
                		show_name = "包含";
                	}else if(operator[i]=='not like'){
                		show_name = "不包含";
                	}else {
                		show_name = "";
                	}
                    $select.append('<option value="'+ (operator[i]) +'">'+ (show_name) +'</option>')
                }
                ...
                var $ok      = $filter.find('button.ok'),
                 ...
                 /** 增加过滤中存在下拉选择的问题 普通都是三个 这个是五个 ，modify by zc 2015-10-15 **/
                 if($selects.length == 5){
                	 $selA    = $selects.first(),
                     $selB    = $selects.eq(3),
                     $andOr   = $selects.eq(2)
                ...
                /** 如果列模型里设置了filterDatas，则把初始值设置到对应的页面元素 modified by zxm 2017-1-9 **/
                if(model.filterDatas){
                	var operatorA = $selA.val(), valA = $valA.val().trim(), operatorB = $selB.val(), valB = $valB.val().trim(), andor = $andOr.val() 
                	if (!valA.length && $valA.attr('data-toggle') != 'selectpicker') {
                		$valA.val(model.filterDatas.valA)
                		$selA.selectpicker('val', model.filterDatas.operatorA)
                		
                		if (!valB.length && $valB.attr('data-toggle') != 'selectpicker') {
                			$valB.val(model.filterDatas.valB)
                			$selB.selectpicker('val', model.filterDatas.operatorB)
                			$andOr.selectpicker('val', model.filterDatas.andor)
                		}
                	}
                	
                	if (!valA.length && $valA.attr('data-toggle') == 'selectpicker') {
                		$selA.selectpicker('val', model.filterDatas.operatorA)
                		$valA.selectpicker('val',model.filterDatas.valA)
                		
                		if (!valB.length && $valB.attr('data-toggle') == 'selectpicker') {
                			$selB.selectpicker('val', model.filterDatas.operatorB)
                			$valB.selectpicker('val', model.filterDatas.valB)
                			$andOr.selectpicker('val', model.filterDatas.andor)
                		}
                	}
                }
                ...
                $ok.click(function() {
                    ...
                    /** 增加对select控件的判断，如果是select控件，即使为空(空表示全部)也要进行过滤 modify by zxm 2016-1-11 **/
                    if (valA.length || $valA.attr('data-toggle')=='selectpicker') {
                        $.extend(filterDatas, {operatorA:operatorA, valA:valA})
                    }
                    if (valB.length || $valB.attr('data-toggle')=='selectpicker') {
                        if (valA.length || $valA.attr('data-toggle')=='selectpicker') $.extend(filterDatas, {andor:andor})
...
//                    }
    ...
    Datagrid.prototype.jumpPage = function(pageCurrent, pageSize) {
        ...
        if (pageSize) {
            ...
            /** 当pageSize大于记录总数时，将当前页号设置为1，modify by zc 2015-10-10 **/
            if (pageSize > paging.total){
            	pageCurrent = 1
            }
        }
    ...
    /** 添加取得表格中指定行数据的API modify by zxm 2015-09-17 **/
    // Api - get row data
    Datagrid.prototype.getRowData = function(index, callback) {
    	if(this.data){
    		callback(this.data[index], this.data.length);
    	} else {
    		callback(null, -1);
    	}
    }
    ...
    Datagrid.prototype.dialogEdit = function($tr, isAdd, data) {
        /** 将下面这段程序，移到4031行 modify by zxm 2015-09-17
    	if (!this.tools.beforeEdit($tr, data)) {
            return false
        }
        **/
...
//增加datagrid，回车可直接查询功能 by 马超 2016/8/16
function select_a(e,t){
    if(e.keyCode == "13"){
        if($(t).parent().hasClass('filter-a')||$(t).parent().hasClass('filter-b')){
            $(t).parent('.filter-a,.filter-b').siblings('.filter-ok').find('.ok').trigger("click");
        }
        else{
            $(t).parent('.wrap_bjui_btn_box').parent('.filter-a,.filter-b').siblings('.filter-ok').find('.ok').trigger("click");
        }
    }
}