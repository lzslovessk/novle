    ...
    $(function() {
        ...
        $.validator && $.validator.config({
            ...
            rules: {
        		...
        		,float: [/^([1-9]\d*\.\d*|0\.\d+|[1-9]\d*|0)$/, '请输入有效的数字'] 
        		...
        		,amountmultiple:[/^[1-9]\d*00$/,'100的整数倍']
        		,startamount:[/^([5-9]\d{2}|[1-9]\d{3,})$/,'500起投']
                ...
                ,accept: function(element, params) {
                ...
                },jsvalide:function(element) {//验证，不包含'"<>?    39 34 60 62 63
                	var validate=true;
                	var value = element.value;
            		if(value.indexOf('\$')>=0){//不能有$ 字符
            			 false;
            		}
            		for ( var i = 0; i < value.length; i++) {
            			if (value.charCodeAt(i) ==39 || value.charCodeAt(i) ==60 || value.charCodeAt(i) ==62 
            					|| value.charCodeAt(i) ==34 || value.charCodeAt(i) ==63) {
            				validate=false;
            			}
            			if(!validate){
            				break;
            		...
            		return (!validate ? this.renderMsg('您输入内容中含有非法字符!') : true);
                }
