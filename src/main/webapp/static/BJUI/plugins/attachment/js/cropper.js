    ...
    renderImage: function (mode) {
      ...
      image.top = min(max(image.top, image._height - image.height), 0);
        ...
        $('.cropper-container').css({
            'maxWidth':parseInt($('.eg-wrapper').css('width')),
            'maxHeight':parseInt($('.eg-wrapper').css('height')),
        ...
        if(round(this.cropper.left)+image.left > 0 && parseInt($('.eg-wrapper').css('width')) <= round(image.width)){
            $('.cropper-container').css({'left':0});
            this.$clone.css({marginLeft: round(this.cropper.left)+image.left});
        }
        else if(round(this.cropper.left)+image.left > 0){
            $('.cropper-container').css({'left':round(this.cropper.left)+image.left});
            this.$clone.css({marginLeft: 0});
        }
        else{
            $('.cropper-container').css({'left':0});
            this.$clone.css({marginLeft: round(this.cropper.left)+image.left});
